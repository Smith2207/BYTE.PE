...
- **Dos formatos de impresión por pedido**: `/pedido/[numero]/boleta` (A4, para guardar como PDF) y `/pedido/[numero]/ticket` (80mm, para ticketera térmica de despacho/empaque) — mismos datos, comparten `numeroComprobante`/`METODO_PAGO_ETIQUETA` de `src/lib/format.ts` para no duplicar el cálculo del correlativo. El ticket fija `@page { size: 80mm auto }` solo en esa ruta para que el driver de la impresora térmica use el ancho de rollo correcto en vez del A4 por defecto del navegador.
