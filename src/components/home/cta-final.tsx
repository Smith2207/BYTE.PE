...
import { CtaDevice } from "@/components/home/cta-device";

export function CtaFinal() {
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      <RevealOnScroll y={32} className="relative overflow-hidden rounded-[2rem] border border-foreground/10 bg-background px-8 py-16 text-foreground sm:px-16">
        <div className="absolute inset-0 scale-110">
          <GlowOrb />
        </div>
        <AtmosphereLayer glowPosition="50% 40%" showRays={false} intensity={0.8} />
        <div className="relative z-10 flex flex-col items-center gap-10 text-center lg:flex-row lg:justify-between lg:text-left">
          <div>
            <h2 className="font-display text-3xl font-semibold tracking-tight sm:text-4xl">
              Tu próximo equipo te espera
            </h2>
            <p className="mx-auto mt-4 max-w-md text-sm text-foreground/70 sm:text-base lg:mx-0">
