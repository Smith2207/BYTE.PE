...
import { ESTADO_RESENA_ETIQUETA } from "./estado-resena-badge";
import type { EstadoResena } from "@/db/schema/enums";

const TODOS = "__todos__";

export function ResenasFiltros() {
  ...
  function actualizar(cambios: { q?: string; estado?: string }) {
    ...
    router.push(qs ? `/admin/resenas?${qs}` : "/admin/resenas");
  }
    ...
    <form onSubmit={onSubmit} className="mb-4 flex flex-wrap items-center gap-3">
      ...
      >
        ...
        <SelectContent>
          ...
          {(Object.keys(ESTADO_RESENA_ETIQUETA) as EstadoResena[]).map((estado) => (
            <SelectItem key={estado} value={estado}>
              {ESTADO_RESENA_ETIQUETA[estado]}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
