...
export const pedidos = pgTable(
  ...
  // Postgres no indexa solo por tener una foreign key — sin esto,
  // "mis pedidos" en /cuenta/pedidos hace un seq scan sobre toda la
  // tabla a medida que crece.
  (t) => ({
    usuarioIdx: index("pedidos_usuario_idx").on(t.usuarioId),
  }),
);
...
export const pedidoItems = pgTable(
  ...
  // pedidoId: se junta con esto para armar CUALQUIER pedido (getPedido,
  // el panel admin, la boleta...). productoId: lo usan los self-joins de
  // "más vendidos" y "comprados juntos" — justo el tipo de consulta que
  // se vuelve lenta primero sin índice.
  (t) => ({
